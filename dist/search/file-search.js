...
export class FileSearchEngine {
    ...
    async searchInRepo(repo, pattern, maxResults) {
        const results = [];
    ...
    async listFiles(repo, path, glob, recursive = true) {
    ...
    async getFile(repo, filePath, startLine, endLine) {
    ...
    async getFileInfo(repo, filePath) {
        if (!isSubPath(repo.path, filePath)) {
            throw new Error('File is not within the repository');
        }
        if (!isValidFile(filePath)) {
