...
server.tool('register_repository', 'Register a local git repository for searching across projects', {
...
server.tool('unregister_repository', 'Remove a repository from the search pool', {
...
server.tool('list_repositories', 'List all registered repositories', {
...
server.tool('get_repository_info', 'Get detailed information about a repository', {
    identifier: z.string().describe('Repository ID, alias, or path'),
}, async ({ identifier }) => {
    try {
...
server.tool('refresh_repository', 'Re-scan a repository to update metadata', {
    identifier: z.string().describe('Repository ID, alias, or path'),
}, async ({ identifier }) => {
    try {
...
// ==================== Text Search Tools ====================
server.tool('search_text', 'Search for text pattern across all registered repositories using ripgrep', {
...
// ==================== Symbol Search Tools ====================
server.tool('find_functions', 'Find function/method definitions across repositories using AST analysis', {
...
server.tool('find_classes', 'Find class definitions across repositories using AST analysis', {
...
server.tool('find_types', 'Find type/interface definitions across repositories using AST analysis', {
    ...
    try {
        ...
        const [typeResults, interfaceResults] = await Promise.all([
            symbolSearch.search({ kind: 'type', name, language, exportedOnly: exportedOnly ?? false, maxResults: maxResults ?? 50 }, repositories),
            symbolSearch.search({ kind: 'interface', name, language, exportedOnly: exportedOnly ?? false, maxResults: maxResults ?? 50 }, repositories),
        ]);
...
// ==================== File Tools ====================
server.tool('get_file', 'Retrieve file contents from any registered repository', {
...
server.tool('list_files', 'List files in a repository path', {
...
server.tool('search_files', 'Find files by name pattern across repositories', {
...
server.tool('get_file_info', 'Get metadata about a file', {
    filePath: z.string().describe('Absolute path to the file'),
}, async ({ filePath }) => {
    try {
        const resolved = repoManager.resolvePath(filePath);
...
server.tool('get_project_structure', 'Get an overview of a repository structure', {
    repoIdentifier: z.string().describe('Repository ID, alias, or path'),
